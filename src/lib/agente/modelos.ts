 ...
 * cualquier `OPENAI_MODEL*` que no tenga entry acá, y el schema de config del
 * agente (`src/lib/validation/agente.schema.ts`) valida `modelo` contra ella.
 * Agregar un modelo nuevo exige agregar su pricing primero — sin eso el kill
 * switch de daily cap quedaría calculando sobre datos inexistentes.
 *
 ...
 *
 * Vive en `lib/` (y no en `server/services/llm/`) porque el schema Zod la
 * necesita y las zonas de ESLint prohíben que `lib/**` importe de
 * `server-services`. `server/services/llm/pricing.ts` re-exporta desde acá
 * para no romper a sus consumidores actuales.
 */
