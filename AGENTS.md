...
**Fase actual:** `Rediseño "sala de control" A-G2 aplicado entero sobre master (2026-08-11). Lo que queda abierto NO es código de pantalla: es método (base de test separada, revisión visual humana) y rendimiento. Retomar: docs/next-session.md.`

**Última acción completada (sesión 2026-08-10/11):** **39 commits sobre master sin pushear** (`d84e9fb..ed50754`). Se cerraron los siete sub-proyectos del rediseño (A-G2), entró el handoff de diseño al repo y se auditaron contra él las pantallas que se habían construido a ciegas. Además, tres tandas que el handoff pedía y los datos no permitían: **instrumentación** (autoría real en `mensajes.sender_user_id`, clasificación del turno en `turn_classifications`, procedencia por campo del Twin, `etapa_alcanzada` para el rail congelado, condiciones de escalado configurables en `agente_config`), **costo de IA** persistido por conversación y por lead (`llm_usage`), y una tanda larga de Inbox y Leads (buscador de conversaciones y del hilo, filtros combinables en la URL, ficha del lead editable, etiquetas, cierre de venta con motivo obligatorio, recordatorios de seguimiento con workflow durable). **12 migraciones nuevas, 33 aplicadas en total.**

**Lo que esta sesión NO hizo y hay que decir en voz alta:** ninguna pantalla se miró con ojos humanos —el panel del navegador no compuso frames en toda la sesión, todo se midió sobre el DOM— y el rendimiento, que es lo primero que pidió el dueño, sigue sin tocarse. Detalle y prioridades en `docs/next-session.md`.
...
3. **Un mock que acepta cualquier schema esconde incompatibilidades con la API real.** `update-lead-twin` nunca completó una ejecución desde Slice 1 sin que nadie lo notara, porque los tests usan `MockLanguageModelV3`.
...
**PENDIENTE USUARIO antes de continuar:** **tres decisiones, ninguna de ellas técnica** — (1) crear un segundo proyecto Supabase para tests, sin el cual `test:integration` no se puede correr; (2) qué puerta de cierre de sesión se queda, el rail del Twin o el botón del header (recomendación: sacar el del header); (3) qué hace el sistema cuando la IA escala a `requiere_humano` y no hay nadie del otro lado. Pendiente manual de siempre: dashboard Supabase → Advisors (CLI 403 en free tier).

**Siguiente sub-paso:** en orden, y con el porqué en `docs/next-session.md`: **(1) proyecto Supabase aparte para tests** —hasta que exista, `test:integration` es un botón de borrar la base y hay contract tests que nunca corrieron—; **(2) revisión visual humana de las pantallas nuevas** —ninguna se miró, y cada captura que mandó el dueño mostró algo que la medición no vio—; **(3) rendimiento de `listActiveLeads`** —es lo primero que pidió el dueño y sigue postergado—; **(4) las dos decisiones del dueño**: qué puerta de cierre de sesión se queda, y qué pasa con las conversaciones escaladas a `requiere_humano` cuando no hay humano.
...
| **Rendimiento**                                            | 🔴 sin empezar | Lo primero que pidió el dueño y lo que más se postergó. Sospechoso concreto: `listActiveLeads` consulta dentro de un loop y el poller la re-ejecuta cada 5 s. Ver `docs/next-session.md`.                                                                                                                                                                                                         |

**Métricas actuales (medidas 2026-08-11 sobre `ed50754`):** **1569/1569 unit tests pass en 129 archivos** · coverage **87.3 / 79.39 / 82.8 / 88.25** (statements/branches/functions/lines; threshold 80/75/80/80; UI client excluida por política browser/E2E) · 0 typecheck errors · 0 lint errors (warnings boundaries pre-existentes) · format clean · **remoto `https://github.com/Leonardo-A1varez/crm.git` (privado), `origin/master` en `d84e9fb` — 39 commits locales sin pushear**.

> ⚠️ **Los integration tests están congelados, no verdes.** `SUPABASE_TEST_URL` apunta al mismo proyecto Supabase que la app, así que `npm run test:integration` vacía la base de dev (ver lección 10). Los contract tests de los repos agregados desde entonces —`turn-classifications`, `llm-usage`, `session-recordatorios`, y los de `agente_config` posteriores a G1— **nunca corrieron contra Postgres real**. Lo último verificado contra Supabase real: RLS 11/11 · leads 16/16 · lead-session 21/21 · productos 20/20 · agente-config 22/22.
...
**Migraciones SQL** (**33 en total, todas aplicadas a Supabase crm-dev**, `supabase/migrations/`. Formato timestamp `YYYYMMDDHHMMSS_<name>.sql`, standard de Supabase CLI v2+). Las 16 de foundation:
...
Las 17 que agregaron los slices 3-4 y el rediseño:
