...
**Fase actual:** `Slice 2 — UI + Realtime + Server Actions (en progreso, 8.2 COMPLETO + validado)`
**Sub-paso actual:** **Slice 2 8.2 conversation view COMPLETO y VALIDADO browser (2026-07-13).** Backend: `MessagesRepository.listBySessionId` (ASC cross-conv, limit conserva N recientes) + `InboxService.getConversation(leadId)` → `ConversationView`. UI: ChannelIcons SVG + MessageBubble (in/out/sistema) + ChatThread (col-reverse bottom-anchored sin JS) + RelativeTime client 30s + ConversationHeader + `inbox/[leadId]/page` RSC + loading skeleton. Validación Playwright contra Supabase real: `/inbox` lista lead fixture + `/inbox/[leadId]` thread 6 mensajes + header canales/stage/relative-time + SideNav aria-current correcto. **Supabase MIGRADO a cuenta main: nuevo `crm-dev` ref `emubzkouwvuzlrtsgorx`** (viejo INACTIVE se auto-borra). 16/16 migrations + types idénticos + integration 157/158 (1 flaky de red, verde en re-run) + keepalive GitHub Action 2x/semana. `.env.local`: keys nuevas `sb_*`; INNGEST/OPENAI/META/UPSTASH con `dev-placeholder` — **valores reales requeridos para 8.4+ (Meta send) y LLM real**. Siguiente: 8.3 TwinPanel (+ `(panel)/error.tsx` pendiente de spec). Pendientes no-blocker: 7.7.B PinoLogger + 7.7.C OTel + 7.7.D Sentry.
