...
**Fase actual:** `Slice 2 vistas — fase 10 Leads EN CURSO 9/11 tasks (2026-07-16): T1-T9 ✅ review clean · T10 merge UI commiteado + E2E 22/22 (falta re-review) · T11 cierre pendiente. Retomar: docs/next-session.md "Opción B-RETOMAR" + ledger .superpowers/sdd/progress.md.`
**Sub-paso actual:** **Fase 9 Productos COMPLETA (subagent-driven, 19 commits `629c647..2d205d9`).** 9.A lista+búsqueda `?q=` (RSC + next/form GET, orden repo nombre+codigo, cap 1000) · 9.B CRUD admin (dialogs Base UI, Server Actions Zod línea 1, codigo*interno inmutable, baja lógica `setProductoActivo`, RLS-denied distinguido de not-found vía probe) · 9.C import CSV (papaparse, preview errores por fila con línea real + fix off-by-one Quotes, confirm upsert por codigo_interno CSV-safe: preserva compatibilidad/imagen_url/activo, defaults DB con `defaultToNull:false`, idempotente) · transversal `getCurrentRol()` (app_metadata, fallback vendedor) + `ilikeContains()` helper PostgREST (escapa `, ( ) % *`— reusar en fase 10 leads). Validado: browser Playwright 9.A+9.B 14/14 + 9.C 11/11 ×2 (re-import idempotente) contra Supabase real · integration productos 20/20 ×2 · CI 709/709 coverage 86/78/84/87. Final whole-branch review (3 Critical fixeados: docs, escape q, shortcut ValidationError; +bounds numéricos Zod↔DB). Backlog diferido →`docs/next-session.md`.
...
**Siguiente sub-paso:** **fase 10 Leads (writing-plans sobre spec `2026-07-14-slice2-vistas-9-12-design.md` §Fase 10 — reusar `ilikeContains` para búsqueda + portar cause-gate de toActionError a inbox como primer item)** **o Slice 4b — deploy + soft launch** (requiere del usuario: creds reales META/OPENAI/INNGEST/UPSTASH en `.env.local`, cuenta Vercel, cuenta Sentry free, número WhatsApp Business). Pendiente manual: dashboard → Advisors (CLI 403 free tier).
...
| **Slice 2 — UI + Realtime + Server Actions**               | 🟡 en progreso | Core 8.x ✅ (inbox+conversación+twin+actions+poller+tabs). **Vistas: fase 9 Productos ✅ 2026-07-15** (`629c647..2d205d9`, plan `docs/superpowers/plans/2026-07-14-slice2-fase9-productos.md`, browser 25 checks + integration 20/20). Fases 10-12 (Leads/Intents+Reglas/Tags+Métricas+Ajustes) pendientes — spec aprobado. |
...
| **Slice 4b — Deploy + soft launch**                        | ⚪ pendiente   | Bloqueado por creds reales usuario (META/OPENAI/INNGEST/UPSTASH/SENTRY) + Vercel + webhook público + templates Meta + monitores + pen test.                                                                                                                                                                                 |

**Métricas actuales:** 709/709 unit tests pass · RLS matriz 11/11 integration · integration productos 20/20 ×2 contra Supabase real (suite completa previa 157/158; timeouts 120s + retry 1) · browser validation 8.1-8.7 + fase 9 (14/14 + 11/11 ×2) ✅ Playwright · 0 typecheck errors · 0 lint errors (warnings boundaries pre-existentes) · format clean · coverage 86.0/78.0/84.5/86.7 (threshold 80/75/80/80) · **remoto `https://github.com/Leonardo-A1varez/crm.git` (privado; local adelantado 19 commits pendiente push)**.
