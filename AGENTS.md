...
**Fase actual:** `Slice 2 — UI + Realtime + Server Actions (en progreso, 8.3 COMPLETO + validado)`
**Sub-paso actual:** **Slice 2 8.3 Lead Twin panel COMPLETO y VALIDADO browser (2026-07-13).** `TwinPanel` server Card con secciones condicionales (consulta/urgencia dot/cotización mono+precio es-locale/bloqueador ámbar/pago+link comprobante/extras jsonb dinámico/context_summary) + `TwinEmptyState` + `StageBadge` color-coded exhaustivo 8 stages (reusado en `InboxListItem`, dedup `STAGE_LABEL` inline) + aside 320px `max-lg:hidden` en `/inbox/[leadId]` + `(panel)/error.tsx` boundary con digest. Validación Playwright contra Supabase real (fixture enriquecida con campos twin): panel full render + lista badge coloreado + 0 console errors. TwinEmptyState y error.tsx verificados solo por typecheck (fixture sin lead sesión-null ni error forzado). **Supabase cuenta main: `crm-dev` ref `emubzkouwvuzlrtsgorx`**, 16/16 migrations, keepalive 2x/semana. `.env.local`: INNGEST/OPENAI/META/UPSTASH con `dev-placeholder` — **valores reales requeridos para 8.4+ (Meta send) y LLM real**. Siguiente: 8.4-8.5 Server Actions write path. Pendientes no-blocker: 7.7.B PinoLogger + 7.7.C OTel + 7.7.D Sentry.
...
**Siguiente sub-paso:** **Slice 2 8.4-8.5 Server Actions write path** (MessageInput + send-message + HandoffToggle + CloseSessionButton). Validar send outbound real requiere `META_*` reales en `.env.local` (hoy `dev-placeholder`). Alternativas: 7.7.B PinoLogger (pre-Slice 4 obligatorio) o Path B smoke.
...
| **Slice 2 — UI + Realtime + Server Actions**               | 🟡 en progreso | 8.1 inbox list ✅ (`11b9e78..f04f382`). 8.2 conversation view ✅ (`6712613..27dd5a7`). 8.3 TwinPanel + error.tsx ✅ validado browser (`ed4a6da..6653be8`). 8.4-8.5 Server Actions write siguiente. 8.6 polling 5s. 8.7 channel tabs. 8.8 tests full. |
