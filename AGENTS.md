...
**Siguiente sub-paso:** en orden, y con el porqué en `docs/next-session.md`: **(1) smoke autenticado** de `inbox_recent_messages` y `transition_handoff`; **(2) QA visual humana** de Leads, Métricas, Agente, Tags e Inbox; **(3) `EXPLAIN (ANALYZE, BUFFERS)`** sobre el volumen disponible; **(4) proyecto Supabase aparte para tests** para descongelar los contratos reales.
