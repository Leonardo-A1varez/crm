...
**Fase actual:** `Slice 2 — UI + Realtime + Server Actions (en progreso, 8.1 COMPLETO)`
**Sub-paso actual:** **Slice 2 8.1 inbox read-only list COMPLETO.** Backend `InboxService.listActiveLeads` orquesta leads+sessions+convs+messages. UI: PanelLayout+SideNav 7 items + inbox/page RSC fetch + InboxList+InboxListItem (canal dots, stage badge, ultima_actividad relativa) + EmptyState + loading.tsx. 597/597 unit verde, coverage 87.71/80.8/84.69/88.77. 12 commits incrementales `11b9e78..f04f382`. Siguiente: 8.2 conversation view. Pendientes no-blocker: 7.7.B PinoLogger + 7.7.C OTel + 7.7.D Sentry (observability prod-grade, ConsoleLogger JSON ya activo) + validación manual browser 8.1.
...
| **Slice 2 — UI + Realtime + Server Actions**               | 🟡 en progreso | 8.1 inbox read-only list ✅ (`11b9e78..f04f382`). 8.2 conversation view siguiente. 8.3 TwinPanel. 8.4-8.5 Server Actions write. 8.6 polling 5s. 8.7 channel tabs. 8.8 tests full.                                              |
...
**Métricas actuales:** 597/597 unit tests pass (Slice 1 base + 4 listActive contract + 6 InboxService) · 154/154 integration tests verde contra Supabase real · 0 typecheck errors · 0 lint errors (warnings boundaries/element-types deprecation pre-existentes) · format clean · coverage 87.71/80.8/84.69/88.77 > threshold 80/75/80/80 · 58 commits conventional history (último: `f04f382 feat(ui): Slice 2 8.1 inbox/page RSC fetch + render`) · **remoto `https://github.com/Leonardo-A1varez/crm.git` configurado (privado, master sync local pendiente push)**.
