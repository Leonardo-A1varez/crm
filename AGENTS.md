...
**Fase actual:** `Slice 2 — UI + Realtime + Server Actions (en progreso, 8.4-8.5 COMPLETO + validado)`
**Sub-paso actual:** **Slice 2 8.4-8.5 Server Actions write path COMPLETO y VALIDADO browser (2026-07-13).** `InboxService.sendMessage/toggleHandoff/closeSession` (delegan en `MetaApiService.sendOutbound` + `HandoffService` + `sessions.close` existentes — cero duplicación outbound) + `inbox.schema.ts` Zod (safeParse línea 1 en 3 actions `_actions/`) + `ActionResult` serializable + `MessageInput` (Enter envía) + `HandoffToggle` + `CloseSessionButton` (dialog resultado + motivo enum condicional) + Toaster sonner en root layout. Client comps reciben action como prop desde RSC (boundaries components↛app). Validado Playwright vs Supabase real: send llega a Graph API real y toast muestra error de token placeholder sin persistir; pause/resume muta badge; close persiste `resultado/motivo_perdida/closed_at` en DB + redirect. 2 fixes en validación: Select uncontrolled→controlled (`value={null}`) + `items` en Root para labels. **Desviaciones spec documentadas:** sin events sin consumer, motivo enum no string, sin idempotencyKey en send manual. Zod 4 `.uuid()` exige RFC 4122 → fixture nueva "María López" con uuid válido (lead legacy `1111...` no pasa schema). `.env.local`: META/OPENAI/INNGEST/UPSTASH `dev-placeholder` — **send outbound real pendiente de `META_*` reales**. Siguiente: 8.6 ConversationPoller. Pendientes no-blocker: 7.7.B PinoLogger + 7.7.C OTel + 7.7.D Sentry.
...
**Siguiente sub-paso:** **Slice 2 8.6 ConversationPoller** (client `router.refresh()` cada 5s) + 8.7 ChannelTabs + 8.8 tests full. Validar send outbound real cuando `META_*` reales estén en `.env.local`. Alternativas: 7.7.B PinoLogger (pre-Slice 4 obligatorio) o Path B smoke.
...
| **Slice 2 — UI + Realtime + Server Actions**               | 🟡 en progreso | 8.1 inbox list ✅ (`11b9e78..f04f382`). 8.2 conversation view ✅ (`6712613..27dd5a7`). 8.3 TwinPanel + error.tsx ✅ (`ed4a6da..6653be8`). 8.4-8.5 Server Actions write ✅ validado browser (`81a44ce..3322ae9`). 8.6 polling 5s siguiente. 8.7 channel tabs. 8.8 tests full. |
...
**Métricas actuales:** 629/629 unit tests pass · 157/158 integration verde contra Supabase nuevo (1 flaky red residencial, verde en re-run; timeouts 120s + retry 1) · browser validation 8.1-8.5 ✅ Playwright · 0 typecheck errors · 0 lint errors (warnings boundaries pre-existentes) · format clean · coverage >threshold 80/75/80/80 · **remoto `https://github.com/Leonardo-A1varez/crm.git` (privado, master sync)**.
