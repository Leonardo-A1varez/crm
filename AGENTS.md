...
**Fase actual:** `Slice 3 — Auth + RLS (COMPLETO + validado; Slice 2 vistas 9-12 diferidas)`
**Sub-paso actual:** **Slice 3 Auth + RLS COMPLETO (2026-07-14).** 9.1: `@supabase/ssr` (upgrade supabase-js 2.110.4) + `src/server/auth/` (client cookies per-request + `updateSession`) + `src/proxy.ts` gate (Next 16, matcher excluye `/api` HMAC) + login email+password (`LoginSchema` TDD, mensaje fijo anti-enumeración) + logout + email en sidebar — validado browser 6/6. 9.2: migration `20260714124024` con 43 policies fail-closed (`TO authenticated` + `is_admin()`/`is_vendedor()` de `app_metadata`, UPDATE con USING+WITH CHECK, infra tables deny-all, storage por bucket) + `MIN_RLS_POLICIES=40` CI + suite `rls-policies.supabase.test.ts` 11/11 (vendedor/admin/anon/sin-rol). 9.3: panel 100% authed — `makeInboxService(db)` + `getInboxServiceForRequest()`, pages+actions swapeados, smoke browser con RLS activo (lista, twin, pause, send fail-fast IG sin config). 9.4: STRIDE post-Slice 3 en threat model + security review (0 findings; nota matcher dot-bypass mitigado por RLS backstop). **Gotcha dev:** suite integration limpia la DB compartida → re-seed fixtures browser tras correrla. Usuario dev: `admin-dev@crm.local` (`app_metadata.rol=admin`). Advisors CLI 403 (free tier) → revisar dashboard manual. Siguiente: Slice 4 (observability 7.7.B/C/D + cron real + `/api/health` + launch) o vistas Slice 2 9-12.
...
**Siguiente sub-paso:** **Slice 4** (cron real wireup + 7.7.B Pino + 7.7.C OTel + 7.7.D Sentry + `/api/health` + soft launch) o vistas Slice 2 9-12. Validar send outbound real cuando `META_*` reales estén en `.env.local`. Pendiente manual usuario: revisar dashboard → Advisors (CLI 403 en free tier).
...
| **Slice 2 — UI + Realtime + Server Actions**               | 🟡 en progreso | 8.1 inbox list ✅ (`11b9e78..f04f382`). 8.2 conversation view ✅ (`6712613..27dd5a7`). 8.3 TwinPanel + error.tsx ✅ (`ed4a6da..6653be8`). 8.4-8.5 Server Actions write ✅ (`81a44ce..3322ae9`). 8.6 poller ✅ (`442e046`) + 8.7 tabs ✅ (`8b6a5b3`) + 8.8 coverage 100% ✅ (`eb3a0c4`). Core 8.x cerrado; vistas 9-12 diferidas. |
...
**Métricas actuales:** 636/636 unit tests pass (inbox service 100% branches) · RLS matriz 11/11 integration · 157/158 integration verde contra Supabase nuevo (1 flaky red residencial, verde en re-run; timeouts 120s + retry 1) · browser validation 8.1-8.7 ✅ Playwright · 0 typecheck errors · 0 lint errors (warnings boundaries pre-existentes) · format clean · coverage >threshold 80/75/80/80 · **remoto `https://github.com/Leonardo-A1varez/crm.git` (privado, master sync)**.
