...
**Métricas actuales (medidas 2026-08-15):** **1717/1717 unit tests pass en 141 archivos** · **4/4 de la suite de contrato contra OpenAI real** · coverage anterior **87.3 / 79.39 / 82.8 / 88.25** (no se recalculó desde 2026-08-13) · 0 errores en ambos typechecks · 0 lint errors (warnings deprecados de boundaries preexistentes) · format clean · **51 migraciones, las 51 aplicadas a `crm-dev`** (repo y ledger cuadran: 51 = 51). Datos en `crm-dev`: 3 leads · 3 vehículos · 7 identificadores · 2 etiquetas · 1 intent · 1 regla de etiquetado · **0 productos, 0 reglas IF/THEN**. Remoto privado: `https://github.com/Leonardo-A1varez/crm.git`; **22 commits sin pushear** en `feat/filtros-leads-y-requiere-humano`.
...
**Migraciones SQL** (**51 en total, las 51 aplicadas a Supabase crm-dev**, `supabase/migrations/`. Formato timestamp `YYYYMMDDHHMMSS_<name>.sql`, standard de Supabase CLI v2+). Las 16 de foundation:
