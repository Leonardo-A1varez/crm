...
**Fase actual:** `Slice 2 — UI + Server Actions (core 8.1-8.8 COMPLETO; vistas 9-12 diferidas)`
**Sub-paso actual:** **Slice 2 core 8.1-8.8 COMPLETO (2026-07-14).** 8.8: +3 edge cases `listActiveLeads` → inbox service 100% (37/37 branches, `eb3a0c4`). 8.6-8.7 validados browser: `RefreshPoller` client (`router.refresh()` 5s, skip tab oculta, re-sync en visibilitychange) montado en `/inbox` y `/inbox/[leadId]` + `ChannelTabs` server como Links `?canal=wa|ig|fb` (sin useSearchParams client) + filtro post-fetch en page con `CanalSchema.safeParse` (param inválido → sin filtro). Validado Playwright vs Supabase real: INSERT externo aparece en thread ≤5s sin F5; `?canal=fb` → EmptyState; `?canal=sms` → todos; 0 console errors. **8.4-8.5 previo:** write path completo (send/handoff/close, delegación services existentes, actions Zod línea 1, toasts curados post security-review `ee7256b`); send real pendiente de `META_*` en `.env.local` (hoy `dev-placeholder`). Fixture browser: Juan Perez legacy `1111...` (uuid no-RFC, zod 4 lo rechaza en actions) + "María López" uuid válido (sesión cerrada perdido/precio en validación 8.5). Siguiente: Slice 3 Auth+RLS (recomendado) o vistas 9-12 o 7.7.B Pino. Pendientes no-blocker: 7.7.B PinoLogger + 7.7.C OTel + 7.7.D Sentry.
...
**Siguiente sub-paso:** decisión usuario — **Slice 3 Auth + RLS** (recomendado: bloqueante launch, spec §9) o vistas Slice 2 9-12 (leads/productos/intents-reglas/tags/métricas/ajustes) o 7.7.B PinoLogger (pre-Slice 4 obligatorio). Validar send outbound real cuando `META_*` reales estén en `.env.local`.
