...
- **El horario de `agente_config` — RESUELTO 2026-08-22: sí lo respeta.** Fuera de horario el nodo de envío no manda y **se pospone hasta la hora hábil siguiente** (`proximaApertura`), en vez de descartar el mensaje: un flujo de seguimiento que caiga siempre de madrugada quedaría mudo y el silencio no se ve en ningún lado. Requiere que `ResultadoAccion` pueda decir "todavía no" (`diferirHasta`) y que el segmento reanude en el MISMO nodo, no en el siguiente.
