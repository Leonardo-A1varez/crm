-- Aprueba un merge como una sola transición Postgres. La implementación
-- anterior hacía siete escrituras independientes desde TypeScript y envolvía
-- el proceso con NoopSessionLock en producción: no había exclusión mutua ni
-- rollback atómico si una escritura intermedia fallaba.
--
-- SECURITY INVOKER conserva RLS. La función además exige rol admin y deriva el
-- actor de auth.uid(): nunca acepta una identidad declarada por el cliente.
...
begin
  ...
  -- Audit primero en orden lógico, pero dentro de la misma transacción: si un
  -- paso posterior falla, el audit también hace rollback y no miente.
...
comment on function public.approve_lead_merge(uuid, uuid) is
  'Merge administrativo atómico: locks ordenados, validaciones, audit, reasignación y delete en una transacción security invoker.';
