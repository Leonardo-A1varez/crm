...
function baseInsert(overrides: Partial<MensajeInsert> = {}): MensajeInsert {
  return {
...
export function runMessagesContract(makeRepo: () => MessagesRepository) {
  describe("MessagesRepository contract", () => {
    ...
    test("create asigna id + created_at + persiste", async () => {
      const m = await repo.create(baseInsert());
    ...
    test("create permite meta_message_id null (outbound antes de delivery)", async () => {
      const m = await repo.create(baseInsert({ meta_message_id: null }));
    ...
    test("findByMetaMessageId localiza por meta id", async () => {
      const m = await repo.create(baseInsert({ meta_message_id: "wamid.xyz" }));
    ...
    test("findByMetaMessageId nunca matchea mensajes con meta_message_id null", async () => {
      await repo.create(baseInsert({ meta_message_id: null }));
    ...
    test("listByConversacion devuelve mensajes de la conv en orden DESC", async () => {
      const m1 = await repo.create(baseInsert({ meta_message_id: "id_1" }));
      await new Promise((r) => setTimeout(r, 5));
      const m2 = await repo.create(baseInsert({ meta_message_id: "id_2" }));
      await new Promise((r) => setTimeout(r, 5));
      const m3 = await repo.create(baseInsert({ meta_message_id: "id_3" }));

      const list = await repo.listByConversacion("conv-1");
      expect(list.map((m) => m.id)).toEqual([m3.id, m2.id, m1.id]);
    });

    test("listByConversacion filtra por conversacion_id", async () => {
      await repo.create(baseInsert({ conversacion_id: "conv-A", meta_message_id: "id_A" }));
      await repo.create(baseInsert({ conversacion_id: "conv-B", meta_message_id: "id_B" }));

      const a = await repo.listByConversacion("conv-A");
      expect(a).toHaveLength(1);
      expect(a[0].conversacion_id).toBe("conv-A");
    });

    test("listByConversacion respeta limit", async () => {
      for (let i = 0; i < 10; i++) {
        await repo.create(baseInsert({ meta_message_id: `id_${i}` }));
        await new Promise((r) => setTimeout(r, 1));
      }
      const list = await repo.listByConversacion("conv-1", { limit: 3 });
      expect(list).toHaveLength(3);
    });

    test("listByConversacion con `before` filtra created_at < before", async () => {
      const a = await repo.create(baseInsert({ meta_message_id: "id_A" }));
      await new Promise((r) => setTimeout(r, 10));
      const b = await repo.create(baseInsert({ meta_message_id: "id_B" }));

      const before = new Date(b.created_at.getTime());
      const list = await repo.listByConversacion("conv-1", { before });
    ...
    test("metadata no comparte ref con storage (defense vs caller mutation)", async () => {
      const m = await repo.create(baseInsert({ metadata: { reply_to: "wamid.original" } }));
    ...
    test("input metadata no compartido con storage (defense vs input mutation post-create)", async () => {
      ...
      const m = await repo.create(baseInsert({ metadata: meta }));
    ...
    test("create rechaza meta_message_id duplicado (UNIQUE 0012)", async () => {
      await repo.create(baseInsert({ meta_message_id: "wamid.dup" }));
      await expect(repo.create(baseInsert({ meta_message_id: "wamid.dup" }))).rejects.toMatchObject(
        {
          code: "CONFLICT",
    ...
    test("create permite múltiples filas con meta_message_id null (partial WHERE NOT NULL)", async () => {
      ...
      const a = await repo.create(baseInsert({ meta_message_id: null }));
      const b = await repo.create(baseInsert({ meta_message_id: null }));
